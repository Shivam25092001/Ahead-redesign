...
import { workSectionData } from "../constants/Work_data";
import UI_Image from "../UI_Image/UI_Image";
import Work_Card from "./WorkCard";

const Work_Section = () => {
  return (
    <section className="md:px-40 w-screen md:w-full  md:h-full px-1 flex justify-center items-center md:mt-36 mt-24">
      <motion.div
          ...
          >
            <div className="flex flex-[0.7] flex-col md:px-6 px-4 py-4 ">
              ...
              <p className="text-start text-[#535353] font-semibold">
                At ahead our goal is to make self - improvement dun and
        ...
        >
          ...
          <div className="flex md:flex-col flex-row md:overflow-y-auto overflow-x-hidden py-8 w-full px-4 mt-4">
            ...
            {workSectionData.map((item, index) => (
              <Work_Card heading={item.heading} info={item.info} key={index} />
            ))}{" "}
          </div>{" "}
        </motion.div>{" "}
      </motion.div>{" "}
